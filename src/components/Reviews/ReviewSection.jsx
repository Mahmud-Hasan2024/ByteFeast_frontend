...
const ReviewSection = () => {
  ...
  const fetchReviews = async () => {
    setLoading(true);
    try {
      const res = await authApiClient.get(`/foods/${productId}/reviews/`);
...
const onSubmit = async (data) => {
  try {
    await authApiClient.post(`/foods/${productId}/reviews/`, data);
    fetchReviews();
  } catch (error) {
  ...
  const handleUpdateReview = async (reviewId) => {
    try {
      await authApiClient.put(
        `/foods/${productId}/reviews/${reviewId}/`,
        editReview
      );
      setEditingId(null);
      fetchReviews();
  ...
  const handleDeleteReview = async (reviewId) => {
    try {
      await authApiClient.delete(`/foods/${productId}/reviews/${reviewId}/`);
      fetchReviews();
    } catch (error) {
      console.log(error);
    }
